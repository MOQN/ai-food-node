...
const INSTRUMENT_MAP = {
  "Chinese Traditional": "Chinese Traditional music with guzheng, erhu, traditional bamboo flutes, and elegant silk-string textures",
  "K-Pop": "a K-Pop song with sleek synthesizers, glittering pop microphones, polished dance beats, and glossy stage accents",
  "R&B": "an R&B song with smooth bass guitars, modern keyboards, silky grooves, and soulful studio polish",
  Jazz: "a Jazz song with saxophones, trumpets, a classy upright bass, and smoky lounge piano touches",
  Acoustic: "an Acoustic song with wooden acoustic guitars, cajons, tambourines, and warm intimate unplugged textures",
  Rock: "a Rock song with electric guitars, heavy drum kits, punchy bass, and bold arena-stage energy",
  Metal: "a Metal song with spiky flying-V guitars, massive double-kick drums, distorted bass, and fierce stage power",
  Funk: "a Funk song with slap bass, wah guitars, tight brass stabs, and sharp syncopated groove",
  "Hip Hop": "a Hip Hop song with turntables, beatpads, gold microphones, deep kicks, and swagger-heavy rhythm production",
  Electronic: "Electronic music with modular synths, drum machines, sequencers, and shimmering digital pulses",
  "8-bit": "8-bit game music with chiptune synth leads, retro pulse-wave arpeggios, pixelated game-console tones, and playful bleeps",
  EDM: "EDM music with DJ decks, neon synthesizers, glowing launchpads, and explosive festival-drop energy",
  Tango: "a Tango song with bandoneon, violin, dramatic piano, and passionate ballroom tension",
  Folk: "a Folk song with acoustic strings, hand percussion, warm flutes, and earthy storytelling textures",
  Opera: "Opera music with orchestral strings, grand piano, theatrical vocals, and sweeping dramatic grandeur",
  Classical: "Classical music with grand piano, violins, orchestral woodwinds, and refined concert hall elegance",
  Afrobeat: "an Afrobeat song with talking drums, layered percussion, rhythmic guitars, and vibrant call-and-response energy",
  Bollywood: "a Bollywood song with tabla, sitar, cinematic string arrangements, and dramatic melodic flourishes",
  Carol: "a Carol song with bells, choir harmonies, gentle acoustic guitars, and festive winter warmth",
  Lullaby: "a Lullaby song with music box plucks, soft piano, airy pads, and delicate bedtime calm",
  Disney: "Disney-style music with bright orchestral brass, playful strings, chimes, and whimsical cinematic sparkle",
  "Children's": "Children's music with toy xylophones, whistles, cheerful claps, and colorful playful singalong energy",
};
...
const IMAGE_REGEN_PROMPT_APPEND =
  ", high-energy action performance, energetic stage presence, dramatic camera pan right";
const ENABLE_MOBILE_UPLOAD_PREVIEW_STEP = true;
...
function determineAudioSettings({ speedMood, vibeMood }) {
  ...
  const keyscaleByVibeMood = {
    ...
    "Cozy & Relaxed": "G major",
    "Just Chilling": "C major",
    "Fresh & Bright": "D major",
    "On Fire!": "E major",
  };
  ...
  return {
    ...
    keyscale:
      keyscaleByVibeMood[vibeMood] || keyscaleByVibeMood[DEFAULT_VIBE_MOOD],
  };
}
...
function applyDefaultMoodSelections() {
  const defaults = [
    {
      containerId: "tags-mood-speed",
      category: "moodSpeed",
      value: DEFAULT_SPEED_MOOD,
    },
    {
      containerId: "tags-mood-vibe",
      category: "moodVibe",
      value: DEFAULT_VIBE_MOOD,
    },
  ];
...
function inviteNextBand() {
  ...
  document
    .querySelectorAll(".loader-text, .circular-loader")
    .forEach((element) => {
...
function buildStructuredLyrics({
  ...
  const tastePhrase = tasteComment
    ? `${tasteLine}, ${tasteComment}`
    : tasteLine;
...
const mobilePreviewNextBtn = document.getElementById("mobile-preview-next-btn");
    ...
    btn.addEventListener("click", () => {
      ...
      if (category === "food" || category === "taste") {
        if (typeof window.updateTextLabels === "function") {
          const labels = [...selections.food, ...selections.taste].slice(0, 8);
...
function handleIncomingMobileBase64Image(base64Image) {
  currentBase64Image = base64Image;

  if (
    ENABLE_MOBILE_UPLOAD_PREVIEW_STEP &&
    mobilePreviewImage &&
    mobilePreviewNextBtn
  ) {
    mobilePreviewImage.src = base64Image;
    showStep("mobile-preview");
    return;
  }
...
// Receive mobile image upload via CustomEvent from main.js
window.addEventListener("mobileImageReceived", (e) => {
  if (e.detail && e.detail.base64) {
    handleIncomingMobileBase64Image(e.detail.base64);
  }
});

if (mobilePreviewNextBtn) {
  mobilePreviewNextBtn.addEventListener("click", () => {
    showStep(1);
  });
}
...
if (generateBtn) {
  generateBtn.addEventListener("click", () => {
    ...
    const instruments =
      INSTRUMENT_MAP[genreStr] || "various musical instruments";
